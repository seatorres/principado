...
//  el usuario ya ha iniciado sesión, a continuación, ejecutar el comando para insertar, actualizar y borrar
            ...
            if(in_array($extension, $allowed_extensions)) {
                ...
                if($ukuran_file <= 1000000) { // Comprueba si el tamaño del archivo subido menos igual a 1 MB
                    ...
                    if(move_uploaded_file($tmp_file, $path)) { // Compruebe si la imagen fue subida o no con éxito
                        ...
                        // consultar comando para guardar los datos en la tabla is_mini
                        $query = mysqli_query($mysqli, "INSERT INTO is_mini(title,image)
    ...
    elseif ($_GET['act']=='update') {
        if (isset($_POST['save'])) {
            if (isset($_POST['mini_id'])) {
                // Captura de datos desde el formulario
                $mini_id       = mysqli_real_escape_string($mysqli, trim($_POST['mini_id']));
                $title              = mysqli_real_escape_string($mysqli, trim($_POST['title']));
                ...
                if (empty($nama_file)) {
                    // consultar comando para cambiar los datos de la tabla is_mini
                    $query = mysqli_query($mysqli, "UPDATE is_mini SET title        = '$title'
                    ...
                    if ($query) {
                        ...
                        header("location: ../../main.php?module=mini&alert=2");
                    } 
                }
                    ...
                    if(in_array($extension, $allowed_extensions)) {
                        ...
                        if($ukuran_file <= 1000000) { // Comprueba si el tamaño del archivo subido menos igual a 1 MB
                            ...
                            if(move_uploaded_file($tmp_file, $path)) { // Compruebe si la imagen subida o no con éxito
                                // Si la imagen ha cargado correctamente, Marca:
                                // consultar comando para cambiar los datos sobre la tabla is_mini
                                $query = mysqli_query($mysqli, "UPDATE is_mini SET title        = '$title',
                                                                                        image        = '$nama_file'
                                                                                  WHERE mini_id = '$mini_id'")
    ...
    elseif ($_GET['act']=='delete') {
        if (isset($_GET['id'])) {
            ...
            // consultar comando para eliminar los datos de la tabla is_mini
            $query = mysqli_query($mysqli, "DELETE FROM is_mini WHERE mini_id='$mini_id'")
            ...
            if ($query) {
                ...
                header("location: ../../main.php?module=mini&alert=3");
            }
        }
    }       
}       
