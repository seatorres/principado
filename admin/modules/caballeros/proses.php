...
//  el usuario ya ha iniciado sesión, a continuación, ejecutar el comando para insertar, actualizar y borrar
            ...
            if(in_array($extension, $allowed_extensions)) {
                ...
                if($ukuran_file <= 1000000) { // Comprueba si el tamaño del archivo subido menos igual a 1 MB
                    ...
                    if(move_uploaded_file($tmp_file, $path)) { // Compruebe si la imagen fue subida o no con éxito
                        ...
                        // consultar comando para guardar los datos en la tabla is_caballeros
                        $query = mysqli_query($mysqli, "INSERT INTO is_caballeros(title,image)
    ...
    elseif ($_GET['act']=='update') {
        if (isset($_POST['save'])) {
            if (isset($_POST['caballeros_id'])) {
                // Captura de datos desde el formulario
                $caballeros_id       = mysqli_real_escape_string($mysqli, trim($_POST['caballeros_id']));
                $title              = mysqli_real_escape_string($mysqli, trim($_POST['title']));
                ...
                // Establecer una ruta de carpeta para almacenar caballeros
                $path               = "../../../img/".$nama_file;
                ...
                if (empty($nama_file)) {
                    // consultar comando para cambiar los datos de la tabla is_caballeros
                    $query = mysqli_query($mysqli, "UPDATE is_caballeros SET title        = '$title'
                                                                      WHERE caballeros_id = '$caballeros_id'")
                                                    or die('Ada kesalahan pada query update : '.mysqli_error($mysqli));

                    // ejecutar consulta
                    if ($query) {
                        // si tiene éxito mensaje indican exitosa actualización de datos
                        header("location: ../../main.php?module=caballeros&alert=2");
                    } 
                }
    ...
    elseif ($_GET['act']=='delete') {
        if (isset($_GET['id'])) {
            $caballeros_id = $_GET['id'];
    
            // consultar comando para eliminar los datos de la tabla is_portfolio
            $query = mysqli_query($mysqli, "DELETE FROM is_caballeros WHERE caballeros_id='$caballeros_id'")
            ...
            if ($query) {
                ...
                header("location: ../../main.php?module=caballeros&alert=3");
            }
        }
    }       
}       
